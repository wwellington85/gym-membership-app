...
function roleAllowsPath(role: Role, pathname: string) {
  ...
  // Front desk: operational set (no settings/more)
  if (role === "front_desk") {
    return (
      pathname === "/dashboard" ||
      pathname.startsWith("/dashboard/") ||
...
export async function middleware(request: NextRequest) {
  ...
  // Role check (expects a staff profile row per user)
  // NOTE: This assumes RLS allows the logged-in user to read their own staff profile.
  ...
  // If we can't determine role, send them to login (or you can send to a "not authorized" page)
...
// Limit middleware to relevant paths (keeps it fast)
export const config = {
  matcher: [
    "/dashboard/:path*",
    "/members/:path*",
